...
function winopen(url)
{
  openWin_W=720;
  openWin_H=480;
  openWin_X=(window.outerWidth-openWin_W)/2
  openWin_Y=(window.outerHeight-openWin_H)/2;
  openWin = window.open( 'https://irenestella.github.io/GMS2.3_Manual/'+url, '', 'width='+openWin_W+', height='+openWin_H+', toolbar=no, menubar=no, location=no, scrollbars=no, left='+openWin_X+', top='+openWin_Y);
  var timer = setInterval(
    function()
    {
      openWin_Count++;
      if (300<openWin_Count)
      {
        clearInterval(timer);
        openWin_Count=0;
      }
      if (0<openWin.document.getElementsByClassName('footer').length)
      {
        openWin.document.getElementById('rh-topic-header').style.display = 'none';
        openWin.document.getElementsByClassName('footer')[0].style.display = 'none';
        clearInterval(timer);
        openWin_Count=0;
      }
    }, 10)
